...
8. When preserving image access, first verify a public HTTPS domain, trusted certificate, AAAA DNS, signed gateway, and expiry. If any check is missing, use text placeholders.
...
- Image URL preservation is opt-in. Enable it only with a public `https://` domain backed by trusted TLS, a private signing key, and the IPv6 image gateway. Never put a bare IPv6 HTTP address or placeholder text into `image_url`.
