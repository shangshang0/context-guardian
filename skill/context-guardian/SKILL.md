...
10. Treat the public Relay as transport, not private storage. For sensitive images, use a user-controlled Relay because the Relay operator can observe transient response bytes.
11. For unknown task failures after compaction, enable message-format preview only for the explicit task. Review the schema-only report; apply repairs only when every issue is losslessly repairable. Enable the live probe only when the user accepts one minimal model request in their current environment.
...
- Message-format live probing is opt-in and consumes one minimal request. It must be ephemeral, use an empty read-only workspace, discard output, and never capture raw TLS bodies, authorization headers, message text, or credentials.
- Do not treat a successful probe as proof that semantic content survived compaction. Stop automatic repair when any schema difference requires guessing.
